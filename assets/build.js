/*
  ...
  * refactored by Maciej Kupisiewicz & Krzysztof Rutecki
  *
  * Released under the MIT license
  * Date: 2025-06-04T21:47Z
  */
 ...
 class Encedo {
    ...
    function checkBrowserSupport(minVersions = { Chrome: 120, Firefox: 133, Safari: 17 }) {
    ...
    if (!window.isSecureContext) {
      console.log("Browser not in secure mode - webcrypto.subtle() is not available!");
    }
    if (!window.crypto || !window.crypto.subtle) {
      console.log("WebCrypto.subtle() is not supported!");
    }

    if (!checkBrowserSupport()) {
      console.log("Browser not supported!");
    } else {
      console.log("WebCrypto API & browser version OK.");
    }
  ...
  async hmacSha256(msgBytes, keyBytes) {
  ...
  async pbkdf2KeyDerive(pass_text, seed_text) {
		try {
      const keySize = 32; // default: 32 bytes
      const iterations = 600000;
      const hashAlg = "SHA-256";

      const enc = new TextEncoder();
      const passwordData = enc.encode(pass_text);
      const saltData = typeof seed_text === "string" ? enc.encode(seed_text) : seed_text;
	...
	async generateSharedSecret(otherPublicKey_Base64) {
  		...
  		const localPubBytes = new Uint8Array(exportedPub);

  		const otherPubBytes = Uint8Array.from(atob(otherPublicKey_Base64), c => c.charCodeAt(0));
  		const otherKey = await crypto.subtle.importKey("raw", otherPubBytes, "X25519", false, []);

  		const sharedBits = await crypto.subtle.deriveBits({ name: "X25519", public: otherKey }, keyPair.privateKey, 256);
  		const sharedSecret = new Uint8Array(sharedBits);

  		return {
    		ephemeralPublicKey: localPubBytes,
  ...
  try {
   		...
   		const passhash = await self.pbkdf2KeyDerive(passU, seed);  //as Uint8Array

			self.log("USER Secret: " + passhash);    
 			var keysU = nacl.box.keyPair.fromSecretKey( passhash );
 			...
 			const jwt = await self.jwt_generate_hs256(header, dataFinal, self.base64Decode(jwt_secret_base64));  
 			self.log(jwt);   
  ...
  (async function() {
  try {
      		const passhash = await self.pbkdf2KeyDerive(pass, seed);  //as Uint8Array
      
 			self.log("Secret: " + passhash);    
 			var keys = nacl.box.keyPair.fromSecretKey( passhash );
 			...
 			var jwt = await self.jwt_generate_hs256(header, data, self.base64Decode(jwt_secret_base64));  
 			self.log(jwt);   
  ...
  try {
      const passhash = await self.pbkdf2KeyDerive(pass, seed);  //as Uint8Array
 			...
 			var user_hmac = self.base64Encode( await self.hmacSha256( self.base64Decode(user_nonce), self.base64Decode(hmac_key) ) );
 	...
 	*/
 	 ...
 	 * @return {string} Encedo USB version name
 	*/ 
 	async jwt_generate_hs256(header_obj, data_obj, secret_bytes) {
 
    var self = this;
    
 	header_obj.alg = "HS256";
	header_obj.typ = "JWT";    
 
    var header_json= JSON.stringify(header_obj);
    var header_utf8Bytes = new TextEncoder().encode(header_json);
    var encodedHeader = self.base64urlEncode(header_utf8Bytes);

    var data_json= JSON.stringify(data_obj);
    var data_utf8Bytes = new TextEncoder().encode(data_json);
    var encodedData = self.base64urlEncode(data_utf8Bytes);
 
	var signature = encodedHeader + "." + encodedData;
    let signature_utf8Bytes = new TextEncoder().encode(signature);
    let signature_hmac;

	try {
        let signature_hmac = await self.hmacSha256(signature_utf8Bytes, secret_bytes );
		let signature_b64 = self.base64urlEncode( signature_hmac );
  		return signature + "." + signature_b64;

	} catch (e) {
		console.error("hmacSha256 error:", e);
		return undefined;
	}
 			...
 			app.scoped('system:config', function(status, data){
 				...
 				if(status) {
 					...
 					app.api('api/system/config').then(function(data) {
						...
						(async function() {
							hash = await app.sha256(req);
							let hash_b64 = app.base64Encode(hash);

							var result = { link: data.link, hash: hash_b64, user: user.user, email: user.email, hostname: user.hostname };
...
/* 	
    ...
    var self = this;
 		...
 		var jwt = jwt_generate_hs256(header, data, self.base64Decode(jwt_secret_base64));  
 
 		//generate MAC
 		var remote_pub2 = new Uint8Array( FromBase64(epk) );
 		var secret_base64 = ToBase64( nacl.scalarMult(aid_key_raw.private, remote_pub2) );
 
 		var mac = CryptoJS.enc.Base64.stringify( CryptoJS.HmacSHA256(jwt + fid, self.toWordArray( self.base64Decode(secret_base64) ) ) );
...
*/
	...
	async function verifyEd25519Signature(pubKeyBytes, message, signatureBytes) {
 	...
 	async function check_log_integrity(sign_key, data){
 		
 		let log_array = data.split("\n");
 
        let keyS = '';
        let cnt = 0;
		console.log('Checking log file integrity...');
 		...
 		for (ident in log_array) {
 			...
 			if ((splitter[2] == 0) && (splitter[3] == 0)) {
				...
				let test = await verifyEd25519Signature(edkey, nonce, nonce_signed);
 			...
 			var key_bytes = app.base64Decode( keyS.replace(/_/g, '/').replace(/-/g, '+') );
			let msg_bytes = new TextEncoder().encode(msg);
			let h = await app.hmacSha256(msg_bytes, key_bytes) ;
			h = h.subarray(0, 16);
 	...
 	*/
 	...
 	function handleMasterPassphrase(result) {
 		...
 		if(result.isConfirmed && nowMasterPassphraseDOMID) {
 			...
 			app.api('api/auth/token').then(function(encedo_authinit){
 				...
 				var jwt = app.jwt_generate_hs256(header, data, self.base64Decode(jwt_secret_base64)); 
 	...
 	register('password_change', function(data){
        ...
        try {
          ...
          const passhash = await self.pbkdf2KeyDerive(passU, seed);  //as Uint8Array
          var keys_newpass = nacl.box.keyPair.fromSecretKey( passhash );
 					...
 					var mac_base64 = self.base64Encode( await self.hmacSha256( self.base64Decode(userNONCE), self.base64Decode(secret_b64) ) );
