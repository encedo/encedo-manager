...
var initMain = function() {
	...
	function checkEncedo() {
		app.ping(function(){
			...
			clearInterval(periodicToWaiter);
	...
	function makeRescue() {
		...
		status_encedoDevice.innerHTML = 'Not connected to Encedo Device yet.';
		status_encedoSoftware.innerHTML = '<p class="animatedX moved">We couldn\'t connect to your device using normal mode so we need to perform next step of our rescue mission! Hang on, we are almost on the finish line!</p><br><h2 class="animatedX moved" id="status_encedoDevice">Please click button below to go to start rescue process.</h2>';
	};
	...
	function makeCheckin() {	
		app.check(function(update){
			...
			setTimeout(function(){ 
				...
				status_encedoSoftware.innerHTML = '<h2 class="animatedX moved" id="status_encedoDevice"><span class="loader-container"><span></span><span></span><span></span><span></span><span></span></span>  &nbsp; Checking software...</h2>';
	...
	register('gotodashboard', function() {
		window.location.replace('https://my.ence.do/');
	});
	
	register('rescue', function() {
		waiterCounter = 0;
		tmpUrl = 'https://' + authenticate_search.value;
		app = new Encedo(tmpUrl);
		periodicToWaiter = setInterval(function(){
			waiterCounter++;
			timerHere.innerHTML = '(' + (30 - waiterCounter) + 's)';
			if(waiterCounter > 29) {
